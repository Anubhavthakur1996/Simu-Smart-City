...
with open(os.path.join(current_dir, "./data/emission_data.json"), "r") as f:
  emissionDict = json.load(f)

# Vehicle agent with hierarchichal Q-tables
class VehicleAgent(Agent):
  def __init__(self, unique_id, model, fType=None):
    ...
    # Assigning fuel type (if not provided, pick randomly)
    if fType is None:
      emissionTableDict = emissionDict
      fuel_keys = list(emissionTableDict.keys())
      if not fuel_keys:
        raise ValueError("No fuel types found in emission_data.json")
      self.fuel_type = random.choice(fuel_keys)
    else:
      self.fuel_type = fType

    # Sample emission profile once for this agent's fuel type
    sampled = EmissionPerStep(self.fuel_type)
    # EmissionPerStep returns fuel_type too; keep consistency
    self.last_fuel_type = sampled.get("fuel_type", self.fuel_type)
    # store pollutant values (per km) without the fuel_type key
    self.emissions = {k: v for k, v in sampled.items() if k != "fuel_type"}
    ...
    self.last_normalized_emission = 0.0
...
class PollutionModel(Model):
  def __init__(self, N, width, height, baseline_pollution=None, policy_engine=None, 
               distance_per_step_km=1.0, cell_length_m=1000.0, mixing_height_m=100.0, fuel_types=None):
    ...
    # flattening fuel types
    fuel_mix_reporters = {
      f"fuel_{ft}": (lambda ft: lambda m: sum(1 for a in m.agents if a.fuel_type == ft))(ft)
      for ft in emissionDict.keys()
    }

    self.datacollector = DataCollector(
      model_reporters=pollutant_reporters | fuel_mix_reporters | {
        # Behavioral metrics
        ...
        # "fuel_mix": lambda m: {ft: sum(1 for a in m.agents if a.fuel_type == ft) for ft in emissionDict.keys()}
      },
      agent_reporters={
    ...
    self.pollution_levels = baseline_pollution.copy() if baseline_pollution else {p:0 for p in UNIT_SCHEMA}

    # Instantiating agents (number of cars)
    for i in range(self.num_agents):
      fType = fuel_types[i] if fuel_types else None   # Setting fuel type for each agent
      a = VehicleAgent(i, self, fType)
      self.agents.add(a)
      x = self.random.randrange(self.grid.width)
      y = self.random.randrange(self.grid.height)
      self.grid.place_agent(a, (x, y))

    # Collect initial state (fleet composition + baseline pollution)
...
def runSimulation(emissionsData=None, cycles=None, numAgents=None, policies=None, activePolicies=None):
  # To initialize agents with sam efuel types for nboth models below
  # Create a reproducible fleet composition
  random.seed(42)  # optional, for reproducibility
  fleet_fuel_types = [random.choice(list(emissionDict.keys())) for _ in range(numAgents)]
  
  # TO verify fleet
  print("Fleet composition:", {ft: fleet_fuel_types.count(ft) for ft in set(fleet_fuel_types)})

  # With baseline pollution levels from synthetic data
  model = PollutionModel(N=numAgents or 10, width=15, height=40, baseline_pollution=baseline_pollution, fuel_types=fleet_fuel_types)
  modelPol = PollutionModel(N=numAgents or 10, width=15, height=40, baseline_pollution=baseline_pollution, 
                            policy_engine=PolicyEngine(active_policies=activePolicies, policies=policies), fuel_types=fleet_fuel_types)
